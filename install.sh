...
if [[ $ACTUAL_PYTHON_V > $REQ_PYTHON_V ]] || [[ $ACTUAL_PYTHON_V == $REQ_PYTHON_V ]];  then 
    ...
    echo -e "\tPython 3.0 is not installed on this machine. Please install Python 3.7 before continuing."
    exit 1
fi

echo -e "\t--Python 3.0 is installed"
...
echo -e "Done."
echo -e "Starting web framework"
echo -e "To exit virtual environment type 'deactivate' in the terminal"

xdg-open http://127.0.0.1:5000/index
flask run
